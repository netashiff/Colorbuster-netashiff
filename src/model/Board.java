...
public class Board {
	...
	public boolean isNorthMatch (int row, int col,  int color) {
		return row >= 0 && row <= rows - 2 && tiles[row + 1][col] != null && tiles[row + 1][col].getColor() == color;
	}
	
	public boolean isSouthMatch (int row, int col, int color) {
		return row >= 1 && row <= rows - 1 && tiles[row - 1][col] != null && tiles[row - 1][col].getColor() == color;
	}
	
	public boolean isEastMatch(int row, int col, int color) {
		return col >= 0 && col <= cols - 2 && tiles[row][col + 1] != null && tiles[row][col + 1].getColor() == color;
	}
	
	public boolean isWestMatch(int row, int col, int color) {
		return col >= 1 && col <= cols - 1 && tiles[row][col - 1] != null && tiles[row][col - 1].getColor() == color;
	}
